...
public class OutputFile {
    ...
    public boolean outputExcel() {
        try {
            ...
            System.out.println("【写入 excel 失败】");
            e.printStackTrace();
        }
        return false;
    }
    ...
    public boolean outputTxt() {
        ...
        try {
            ...
            System.out.println("【成功写入 txt】");
            return true;
        } catch (Exception e) {
            System.out.println("【写入 txt 失败】");
            e.printStackTrace();
        }
        return false;
    }
    ...
    public boolean outputXml() {
        try {
            ...
            System.out.println("【成功写入 xml】");
            return true;
        } catch (Exception e) {
            System.out.println("【写入 xml 失败】");
            e.printStackTrace();
        }
        return false;
    }
    ...
    public boolean outputJson() {
        try {
            ...
            for (StudentInfo studentInfo : studentList) {
                ...
                info.put("name", studentInfo.getName());
                info.put("gender", studentInfo.getGender());
                info.put("course", studentInfo.getCourse());
                info.put("grade", studentInfo.getGrade());
                String singleInfoStr = new JSONObject(info).toString();
                // System.out.println(singleInfoStr);
                if (i < lenth - 1) {
                    bufferedWriter.write(singleInfoStr + ",");
                } else {
                    bufferedWriter.write(singleInfoStr);
                }
                i++;
            ...
            fileOutputStream.close();
            System.out.println("【成功写入 json】");
            return true;
        } catch (Exception e) {
            System.out.println("【写入 json 失败】");
            e.printStackTrace();
        }
        return false;
    }
