...
public class Menu {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 6: // 成绩查询
                    ...
                    break;
                case 7:
                    saveExcel("data/student.xls", studentList.getStudentList());
                    break;
                case 8:
                    saveTxt("data/student.txt", studentList.getStudentList());
                    break;
                case 9:
    ...
    public void saveExcel(String pathExcel, ArrayList<StudentInfo> studentList) {
        OutputFile outputFile = new OutputFile(studentList);
        outputFile.setPathExcel(pathExcel);
        outputFile.outputExcel();
    }

    public void saveTxt(String pathTxt, ArrayList<StudentInfo> studentList) {
        OutputFile outputFile = new OutputFile(studentList);
    ...
    public void inputFromKeyboard(StudentList studentList) {
        ...
        String inputLine = "5 小李 男 物理 88";
        // String splitSymbol = " ";
        while(!inputLine.equals(endstr)) {
            String[] info = inputLine.split(" ", 5);
            // System.out.println(info[0] + info[1] + info[2] + info[3] + info[4]);
            System.out.println(info[0]);
            // StudentInfo studentInfo = new StudentInfo(info[0], info[1], info[2], info[3], Float.parseFloat(info[4]));
